...
public class MtrUtil {
    public static void syncTrainToPlayers(TrainServer trainServer, List<ServerPlayer> players) {
        ...
        for(ServerPlayer player : players) {
            Registry.sendToPlayer(player, PACKET_UPDATE_TRAINS, packet);
        }
    }
    ...
    public static Tuple<MutableComponent, MutableComponent> getInterchangeRouteNames(Station station, Route thisRoute, RailwayData data, ChatFormatting forceColor) {
        MutableComponent chinTexts = Component.literal("");
        MutableComponent engTexts = Component.literal("");
        MutableComponent comma = Component.literal(", ");
        ...
        for(Route route : routeInSta.values()) {
            MutableComponent chinText = Component.literal(getRouteName(getNameChinese(route.name)));
            MutableComponent engText = Component.literal(getRouteName(getNameEnglish(route.name)));
            if(forceColor == null) {
        ...
        for(Siding siding : railwayData.sidings) {
            ...
            for(TrainServer train : ((SidingAccessorMixin)siding).getTrains()) {
                ...
                for (int i = 0; i <= train.trainCars; i++) {
                    final int car = ((TrainAccessorMixin) train).getReversed() ? train.trainCars - i : i;
                    final int spacing = train.spacing;
                    positions[i] = ((TrainAccessorMixin)train).getTheRoutePosition(car * spacing);
                }
